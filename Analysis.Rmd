...
```{r}
leveneTest(AggregateScore ~ ParentEduc, data = cleaned_df)
```
...
```{r}
kruskal.test(AggregateScore ~ ParentEduc, data = cleaned_df)
```
## Linear Models
...
```{r}
library(ranger)
rf_step <- ranger(AggregateScore ~ Male + ParentEduc + LunchDiscount + TestPrep + PracticeSport + IsFirstChild + NrSiblings + PrivateTransport + WklyStudyHours + EthnicGroupA + EthnicGroupB + EthnicGroupC + EthnicGroupD + EthnicGroupE + ParentDivorced + ParentMarried + ParentSingle + ParentWidowed, data = df.train, importance = "impurity")
rf_step
#rf_all <- ranger
```
